...
; Helpers
(define (square x) (expt x 2)) ;;;;;gibt es doch die Standardfunktion (sqr x) für
...
;;;;;
;Schön, dass ihr an die division by zero gedacht habt
;3/3 Punkten
;;;;;
...
(define (Anfangskurs Ab Al Bb Bl eastwards)
  (let* ([a (radians->degrees (ar 
                               ...
                               (degrees->radians Bb)
                               (degrees->radians Bl)))])
    (if eastwards a (360 - a)))) ;;;;;Die Bestimmung ob der Kurs östlich oder westlich ist sollte
                                 ;;;;;selbstständig erfolgen und nicht als Parameter übergeben werden
                                 ;;;;;Außerdem ist der false-Zweig nicht funktionsfähig, da dabei
                                 ;;;;;Infix Notation angewandt wurde
...
;;;;;
;Zwar sind Vektoren eigentlich noch nicht behandelt aber okay
;Schöne Ausfahlbehandlung
;6/6
;
